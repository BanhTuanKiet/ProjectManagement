...
export default function TaskSupport({ open, onClose, projectId, task }: Props) {
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            console.error("Error when sending support email:", err)
        } finally {
            setSending(false)
        }
    }
            ...
            <div className="relative max-w-2xl w-full bg-white rounded-lg shadow-2xl p-6 z-10 animate-fade-in">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                        ...
                        <div className="font-medium text-gray-800">{task.assignee ?? task.assigneeId ?? "None"}</div>
                    </div>
                    <div>
                    ...
                    <div className="col-span-2">
                        ...
                        <div className={severityStyle}>
                            ...
                            {isExpired ? " — EXPIRED" : isNear ? ` — ~${daysLeft} DAYS LEFT` : ""}
                        </div>
                    </div>
                ...
                <div className="mt-6 border-t pt-4">
                    <div className="text-sm font-medium text-gray-700 mb-2">Support / Feedback Content</div>
                    <textarea
                        value={content}
                        ...
                        placeholder={
                            isExpired
                                ? "Describe the issue, overdue reasons, and required support..."
                                : "Write what you want to ask about the task's progress (e.g., Are you facing any difficulties?)..."
                        }
                    />
                </div>
                ...
                <div className="mt-4 flex items-center justify-between">
                    <div className="text-sm">
                        {isExpired
                            ? <span className="text-red-600 font-bold">REQUIRES IMMEDIATE ACTION</span>
                            : isNear ? <span className="text-yellow-700 font-bold">NEAR DEADLINE</span>
                                : <span className="text-gray-500">Normal</span>}
                    </div>

                    <div className="flex items-center gap-2">
                        {result && (
                            <div className={`text-sm ${result.includes("success") ? "text-green-600" : "text-red-600"}`}>
                                {result}
                            </div>
                        )}
                        <button
